...
import { Select, Button, DatePicker, InputNumber } from "antd";
...
const Tickets = () => {
  const { tickets, companies } = useInfoContext();
  ...
  const filteredTickets = useMemo(() => {
    ...
    if (filters.dateFrom) {
      result = result.filter(
        (t) => new Date(t.departureDate) >= filters.dateFrom.toDate()
      );
    }
    if (filters.dateTo) {
      result = result.filter(
        (t) => new Date(t.departureDate) <= filters.dateTo.toDate()
      );
    }
